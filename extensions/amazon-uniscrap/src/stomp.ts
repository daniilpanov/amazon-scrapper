...
export default class StompConnection {
    ...
    public connect(err?: any): void | Promise<void> {
        if (!this.needReconnect || this.reconnectionCount >= this.maxReconnectionCount) {
            if (this.resetNeedReconnect) this.needReconnect = true;
        ...
        return new Promise(async (resolve, reject): Promise<void> => {
            if (err) console.log('Reconnecting due to the error:', err);
    ...
    public subscribe(queue: string, callback: TaskHandler, prefetch_count?: number, params?: {
        [key: string]: string,
    }): void {
        if (!params) params = {};
        if (!prefetch_count) prefetch_count = this.maxConcurrentTasks;
        ...
        if (this.subscriptions.has(queue)) {
            ...
            // Если клиент подключен и подписка неактивна, создаем новую
    ...
    public unsubscribe(queue: string): void {
        if (!this.subscriptions.has(queue)) return;
    ...
    private async handleMessage(message: any, callback: TaskHandler): Promise<void> {
        this.reconnectionCount = 0;
        ...
        try {
            ...
            let res;
            res = await callback.bind(this)(data);
    ...
    public sendMessage(queue: string, message: any, headers?: { [key: string]: string }): void {
        ...
        try {
            if (typeof message === 'string') this.stompClient.publish({
                destination: queue,
                headers: headers,
                body: message,
            });
