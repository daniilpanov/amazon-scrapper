const endpoint = 'http://localhost:8832/helium/set_100asins';
...
chrome.runtime.onMessageExternal.addListener(async (message, sender, sendResponse) => {
    ...
    let res = await fetch(
        'http://localhost:8832/tasks/acquire/100asins/' + message.header_id + '/' + message.task_id,
        {method: 'post'},
    )
    if (res.status === 200) {
        sendResponse('OK');
        ...
        if (!res) {
            fetch('http://localhost:8832/tasks/report/' + message.task_id, {
                headers: {
