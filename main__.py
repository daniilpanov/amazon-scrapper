...
if __name__ == '__main__':
    ...
    def gather(script, args, statuses, number=None):
        def wrapper():
            ...
            while res in ('captcha', 'error', 'reload') and lim > 0:
                print(f'reload {statuses} gathering to directory ' + folder + str(number))
                process = subprocess.Popen(
                    [os.path.join('venv', 'Scripts', 'python.exe'), f'requests/{script}_collect.py', '--start', *args],
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE
                )
            ...
            if res == 'error' or lim <= 0:
                print(f'an error occurred in {statuses} gathering to directory {folder}{number}...')
            elif res == 'success':
                print(f'{statuses} collected to directory ' + folder + str(number))
            else:
                print(f'unknown status: {statuses} gathering to', folder + str(number) + ':', res)
            return res == 'success'
    ...
    if '5' in switches:
        ...
        def multithread_gather(number, asins):
            if not os.path.exists(folder + str(number)):
                os.mkdir(folder + str(number))
            return gather('reviews', [folder + str(number), ','.join(asins)], 'products\' reviews', number)
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for i in range(len(features)):
                ...
                if r:
                    if first:
                        shutil.copyfile(
                            os.path.join(folder + str(i), 'reviews_list.csv'),
                            os.path.join(folder, 'reviews_list.csv'),
                        )
                        first = False
                    else:
                        f = open(os.path.join(folder, 'reviews_list.csv'), 'a')
                        f2 = open(os.path.join(folder + str(i), 'reviews_list.csv'), 'r')
                        lines = f2.readlines()[1:]
                        f2.close()
                        f.writelines(lines)
                        f.close()
