...
server_host = ('https://' if os.environ.get('HTTPS_READY') == 'true' else 'http://') + os.environ.get('HOST', 'localhost')
...
def generate_update_xml():
    ...
    for folder in os.listdir('compiled_ext/'):
        if folder.endswith('.crx'):
            manifest_path = os.path.join('ext', folder.replace('.crx', ''), 'manifest.json')
            if os.path.exists(manifest_path):
                version = folder.split ('-')[-1].replace('.crx', '')  # Предполагаем, что версия в имени файла
                app_id = generate_extension_id(manifest_path)  # Генерируем ID расширения
                codebase = f"{server_host}:{this_port}/ext/get/{folder}.crx"
                ...
                update_file_path = os.path.join(update_metadata_path, f"{app_id}.xml")
                tree = ET.ElementTree(root)
                tree.write(update_file_path, encoding='utf-8', xml_declaration=True)

@router.get('/update/{app_id}.xml', response_class=FileResponse)
def get_update_xml(app_id: str):
    update_file_path = os.path.join('update_metadata/', f"{app_id}.xml")
...
def get_update_xml(app_id: str):
    update_file_path = os.path.join('compiled_ext/', f"{app_id}.crx")
    if os.path.exists(update_file_path):
        return FileResponse(update_file_path)
    else:
        raise HTTPException(status_code=404, detail="Update file not found")
