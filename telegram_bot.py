...
        keyboard.add(key) #добавляем кнопки в клавиатуру
    
    return keyboard
...
    # Processing code
    
    send_msg(user_id, f'Finished ASINs {asins} processing')
...
    
    
def format_link(content, url):
    url = remove_url_tcp(url)
    if content == None:
        content = url
        
    return telebot.formatting.hlink(content, url, escape=False)
    
...
        process_asins(call.message.chat.id)
